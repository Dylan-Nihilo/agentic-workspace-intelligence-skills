...
```bash
npm run --silent understanding:harness -- scout --repo <repo-path> --out <package-dir>
npm run --silent understanding:harness -- ingest-scout --package <package-dir> --analysis <package-dir>/scout/output.json
```

`scout` 只生成 `scout/context.json` 与 `scout/request.md`,其中静态 inventory/code-map 只是给 L0 worker 的证据材料。必须让 `repo-scout` worker 读取 request 后写 `repo-scout-agent-output/v1` 到 `scout/output.json`,再用 `ingest-scout` 接受。不要把 `scout/deterministic-hints.*.json` 当最终 L0 结果;harness 会拒收 `deterministic-baseline`。
...
`analyze` 默认要求 package 里已有 agent scout 接受后的 `repo-profile.json` 与 `scan-policy.json`;缺失时直接失败。进入 dispatch 前必须检查 L0 识别:若 `repoKind`、`primaryLanguage`、`frameworks` 与仓库明显冲突,先停下修 profile/routing 问题,不要把后端仓库按前端默认继续扫。`--allow-baseline-scout` 只用于兼容性测试,不用于真实 repo 理解。
...
5. 每次 `ingest` 返回 `{merged:false, issues:[...]}` 时,把 issues 原样交回对应 worker 修正,最多 2 次;仍失败时用 open-question 原语留痕并上报 rejected:

```bash
npm run --silent understanding:harness -- ingest --package <package-dir> --open-question "<question>" --tasks "<task-id>,<node-id>"
```
6. 最终综合(L4):按 `repo-synthesizer` skill 写入 `analyses/repo-understanding.json`。综合**必须包含 `businessDomains`**(按业务而非 router 文件分组的域),它是人读页面的业务分类来源。
7. 呈现(闭环收尾,确定性渲染):综合写回后,渲染人可消费的 HTML 页面。它是消费投影,确定性读取 fact-graph 与综合结果,不做 LLM 即兴生成。**这一步不可跳过——流程必须闭环到人能打开的页面。**
