...
(function(){
  ...
  function setSemanticTerritoryFocus(zoneId){var layers=flowHost.querySelectorAll('.semantic-territory-layer');if(!layers.length)return;layers.forEach(function(layer){layer.classList.toggle('semantic-territory-layer--focus',Boolean(zoneId));layer.querySelectorAll('[data-territory-zone]').forEach(function(group){group.classList.toggle('semantic-territory--active',group.dataset.territoryZone===zoneId);});});}
  function drawSemanticTerritories(visible){flowHost.querySelectorAll('.semantic-territory-layer').forEach(function(layer){layer.remove();});var regions=buildVisibleRepositoryRegions(visible);if(regions.length<2)return;var cards=new Map(Array.from(flowHost.querySelectorAll('[data-node-card]')).map(function(card){return [card.dataset.nodeCard,card];}));if(!cards.size)return;var hostRect=flowHost.getBoundingClientRect();var width=Math.max(flowHost.scrollWidth,flowHost.offsetWidth);var height=Math.max(flowHost.scrollHeight,flowHost.offsetHeight);var shapeSvg=svgNode('svg',{class:'semantic-territory-layer semantic-territory-layer--shapes',width:width,height:height,viewBox:'0 0 '+width+' '+height,'aria-hidden':'true'});var labelSvg=svgNode('svg',{class:'semantic-territory-layer semantic-territory-layer--labels',width:width,height:height,viewBox:'0 0 '+width+' '+height,'aria-hidden':'true'});regions.forEach(function(region,regionIndex){var regionCards=region.paths.map(function(filePath){return cards.get(filePath);}).filter(Boolean);if(!regionCards.length)return;var bounds=regionCards.map(function(card){return semanticTerritoryCardBounds(card,hostRect);});var className='semantic-territory semantic-territory--tone-'+(regionIndex%6);var shapeGroup=svgNode('g',{class:className,'data-territory-zone':region.zone.zoneId});if(bounds.length===1){var bound=bounds[0];shapeGroup.appendChild(svgNode('ellipse',{class:'semantic-territory__shape',cx:bound.left+bound.width/2,cy:bound.top+bound.height/2,rx:bound.width/2+22,ry:bound.height/2+18}));}else{var padding=20;var hull=semanticTerritoryHull(bounds.flatMap(function(bound){return [{x:bound.left-padding,y:bound.top-padding},{x:bound.right+padding,y:bound.top-padding},{x:bound.right+padding,y:bound.bottom+padding},{x:bound.left-padding,y:bound.bottom+padding}];}));shapeGroup.appendChild(svgNode('path',{class:'semantic-territory__shape',d:roundedTerritoryPath(hull)}));}shapeSvg.appendChild(shapeGroup);var minX=Math.min.apply(Math,bounds.map(function(bound){return bound.left;}));var minY=Math.min.apply(Math,bounds.map(function(bound){return bound.top;}));var labelGroup=svgNode('g',{class:className,'data-territory-zone':region.zone.zoneId});var label=svgNode('text',{class:'semantic-territory__label',x:Math.max(8,minX+4),y:Math.max(12,minY-8)});label.textContent=(model.repositoryZones.preview?'草稿 · ':'')+region.zone.label+' · '+regionCards.length;labelGroup.appendChild(label);labelSvg.appendChild(labelGroup);});flowHost.prepend(shapeSvg);flowHost.appendChild(labelSvg);}
