...
(function(){
  ...
  function selectStage(stageNumber,scrollToAtlas){selectedStage=Math.max(1,Math.min(model.stages.length,stageNumber));renderStages();renderStageDetail(model.stages[selectedStage-1]);renderEvolution();renderDomainControls();renderInspector();renderFlow(true);if(scrollToAtlas)document.getElementById('atlas').scrollIntoView({behavior:'smooth',block:'start'});}
  ...
  function renderDomainControls(){
    var active=selectedStage===7;var hasZones=model.repositoryZones.zones.length>0;document.querySelector('.flow-guide').textContent=!active?'双指捏合缩放 · 三指/鼠标拖拽移动 · 点“展开”继续一层':hasZones?(model.repositoryZones.preview?'沿用 S6 树形结构 · 当前为 Agent 草稿预览，尚未通过独立审核':'沿用 S6 树形结构 · Agent 领域随展开和收起实时重排'):'沿用 S6 树形结构 · 等待 Domain Agent 提出并审核领域划分';
  }
  ...
  function renderFlowBranch(filePath,connection,depth,ancestors,seen,stats){
    ...
    var children=connections(filePath);var terminal=Boolean(state);var expanded=!terminal&&expandedNodes.has(filePath)&&children.length>0;var limit=childLimits.get(filePath)||CHILD_BATCH;var shown=expanded?children.slice(0,limit):[];if(selectedStage===7&&model.repositoryZones.zones.length)shown=arrangeVisibleStageSevenChildren(shown);stats.nodes+=1;stats.maxDepth=Math.max(stats.maxDepth,depth);
    var zoneMeta=selectedStage===7?repositoryZoneMeta(filePath):null;var cardClass='flow-card flow-card--kind-'+escapeHtml(node.kind)+(depth===0?' flow-card--root':'')+(selectedPath===filePath?' flow-card--selected':'')+(terminal?' flow-card--terminal':'')+(zoneMeta?' semantic-zone-node semantic-zone-node--tone-'+zoneMeta.tone:'');
  ...
  function arrangeVisibleStageSevenChildren(items){var zoneOrder=new Map(model.repositoryZones.zones.map(function(zone,index){return [zone.zoneId,index];}));return items.map(function(item,index){var meta=repositoryZoneMeta(item.path);return {item:item,index:index,zoneIndex:meta?(zoneOrder.get(meta.zone.zoneId)||0):999,subzoneId:meta?meta.subzone.subzoneId:''};}).sort(function(a,b){return a.zoneIndex-b.zoneIndex||a.subzoneId.localeCompare(b.subzoneId)||a.index-b.index;}).map(function(entry){return entry.item;});}
  function collectVisibleRepositoryGraph(){var paths=[];var pathSet=new Set();var edges=[];var references=0;var maxDepth=0;var depthByPath=new Map();function visit(filePath,depth,ancestors){var node=nodeById.get(filePath);if(!node)return;if(pathSet.has(filePath)){references+=1;return;}pathSet.add(filePath);paths.push(filePath);depthByPath.set(filePath,depth);maxDepth=Math.max(maxDepth,depth);var children=connections(filePath);if(!expandedNodes.has(filePath)||!children.length)return;var limit=childLimits.get(filePath)||CHILD_BATCH;var nextAncestors=new Set(ancestors);nextAncestors.add(filePath);children.slice(0,limit).forEach(function(connection){edges.push({source:filePath,target:connection.path,connection:connection});if(nextAncestors.has(connection.path)||pathSet.has(connection.path)){references+=1;return;}visit(connection.path,depth+1,nextAncestors);});}if(rootPath)visit(rootPath,0,new Set());return {paths:paths,pathSet:pathSet,edges:edges,references:references,maxDepth:maxDepth,depthByPath:depthByPath};}
  ...
  function setSemanticTerritoryFocus(zoneId){var layer=flowHost.querySelector('.semantic-territory-layer');if(!layer)return;layer.classList.toggle('semantic-territory-layer--focus',Boolean(zoneId));layer.querySelectorAll('[data-territory-zone]').forEach(function(group){group.classList.toggle('semantic-territory--active',group.dataset.territoryZone===zoneId);});}
  function drawSemanticTerritories(visible){flowHost.querySelectorAll('.semantic-territory-layer').forEach(function(layer){layer.remove();});var regions=buildVisibleRepositoryRegions(visible);if(regions.length<2)return;var cards=new Map(Array.from(flowHost.querySelectorAll('[data-node-card]')).map(function(card){return [card.dataset.nodeCard,card];}));if(!cards.size)return;var hostRect=flowHost.getBoundingClientRect();var width=Math.max(flowHost.scrollWidth,flowHost.offsetWidth);var height=Math.max(flowHost.scrollHeight,flowHost.offsetHeight);var svg=svgNode('svg',{class:'semantic-territory-layer',width:width,height:height,viewBox:'0 0 '+width+' '+height,'aria-hidden':'true'});regions.forEach(function(region,regionIndex){var regionCards=region.paths.map(function(filePath){return cards.get(filePath);}).filter(Boolean);if(!regionCards.length)return;var bounds=regionCards.map(function(card){return semanticTerritoryCardBounds(card,hostRect);});var group=svgNode('g',{class:'semantic-territory semantic-territory--tone-'+(regionIndex%6),'data-territory-zone':region.zone.zoneId});if(bounds.length===1){var bound=bounds[0];group.appendChild(svgNode('ellipse',{class:'semantic-territory__shape',cx:bound.left+bound.width/2,cy:bound.top+bound.height/2,rx:bound.width/2+22,ry:bound.height/2+18}));}else{var padding=20;var hull=semanticTerritoryHull(bounds.flatMap(function(bound){return [{x:bound.left-padding,y:bound.top-padding},{x:bound.right+padding,y:bound.top-padding},{x:bound.right+padding,y:bound.bottom+padding},{x:bound.left-padding,y:bound.bottom+padding}];}));group.appendChild(svgNode('path',{class:'semantic-territory__shape',d:roundedTerritoryPath(hull)}));}var minX=Math.min.apply(Math,bounds.map(function(bound){return bound.left;}));var minY=Math.min.apply(Math,bounds.map(function(bound){return bound.top;}));var label=svgNode('text',{class:'semantic-territory__label',x:Math.max(8,minX+4),y:Math.max(12,minY-8)});label.textContent=(model.repositoryZones.preview?'草稿 · ':'')+region.zone.label+' · '+regionCards.length;group.appendChild(label);svg.appendChild(group);});flowHost.prepend(svg);}
  function bindSemanticTerritories(){flowHost.querySelectorAll('[data-zone-id]').forEach(function(card){card.addEventListener('mouseenter',function(){setSemanticTerritoryFocus(card.dataset.zoneId);});card.addEventListener('mouseleave',function(){setSemanticTerritoryFocus(null);});card.addEventListener('focusin',function(){setSemanticTerritoryFocus(card.dataset.zoneId);});card.addEventListener('focusout',function(){setSemanticTerritoryFocus(null);});});}
  function renderRepositoryZones(centerRoot){if(!rootPath||!nodeById.has(rootPath)){flowHost.innerHTML='<div class="flow-empty"><strong>没有可展示的文件关系</strong><p>当前关系筛选下没有文件级节点。</p></div>';return;}var visible=collectVisibleRepositoryGraph();var regions=buildVisibleRepositoryRegions(visible);var stats={nodes:0,references:0,maxDepth:0};flowHost.innerHTML='<div class="semantic-tree-layout">'+renderFlowBranch(rootPath,null,0,new Set(),new Set(),stats)+'</div>';bindFlow();bindSemanticTerritories();document.getElementById('graph-caption').textContent=(model.repositoryZones.preview?'待审核草稿 · ':'')+rootPath+' · 树形语义布局 · '+format(visible.paths.length)+' 个可见节点 / '+format(visible.references)+' 个共享引用 / '+format(regions.length)+' 个当前区域 / '+(visible.maxDepth+1)+' 层';requestAnimationFrame(function(){drawSemanticTerritories(visible);drawSharedLinks();if(centerRoot)centerCameraOnPrimary();else applyCamera();});}
  ...
  function renderFlow(centerRoot){
    ...
    if(selectedStage===7&&model.repositoryZones.zones.length){renderRepositoryZones(centerRoot);return;}
  ...
  new ResizeObserver(function(){var next={width:flowViewport.clientWidth,height:flowViewport.clientHeight};if(lastViewportSize.width){camera.x+=(next.width-lastViewportSize.width)/2;camera.y+=(next.height-lastViewportSize.height)/2;applyCamera();}lastViewportSize=next;requestAnimationFrame(function(){if(selectedStage===7&&model.repositoryZones.zones.length)drawSemanticTerritories(collectVisibleRepositoryGraph());drawSharedLinks();});}).observe(flowViewport);
